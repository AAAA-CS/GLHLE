...
def data_processing(Dataset='UP', batch_size=64, PATCH_LENGTH=11, flag='Train', Train_size=76, noise_type='sym',
                    noise_ratio=24):
    ...
    if flag == 'train':
        ...
        train_iter, valid_iter, test_iter = generate_iter(train_gt, train_indices, test_gt, test_indices,
                                                          train_indices.shape[0],
                                                          PATCH_LENGTH, padded_data, INPUT_DIMENSION, batch_size,
                                                          noise_type, noise_ratio)

        return train_iter, valid_iter, test_iter, train_indices
...
class HSIDA(Data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'preTrain':
            ...
            return raw, img1, img2, target, index
        elif self.mode == 'test':
            img = torch.from_numpy(image.transpose(2, 0, 1)).type(torch.FloatTensor)
            return img, target, index
...
def generate_iter(label_train, train_indices, y_test, test_indices, VAL_SIZE,
    ...
    y_traingt = np.array(label_train)
    ...
    torch_dataset_train = HSIDA(images=x_train, labels=torch.from_numpy(y_train).type(torch.LongTensor),
                                labels_gt=torch.from_numpy(y_traingt).type(torch.LongTensor), mode='train')

    torch_dataset_valida = HSIDA(images=x_val, labels=torch.from_numpy(y_val).type(torch.LongTensor),
                                 labels_gt=torch.from_numpy(y_val).type(torch.LongTensor), mode='test')

    torch_dataset_test = HSIDA(images=x_test, labels=torch.from_numpy(y_test).type(torch.LongTensor),
                               labels_gt=torch.from_numpy(y_test).type(torch.LongTensor), mode='test')
...
def generate_all(true_point, patch, mirror_image, band, batch_size=128):
    ...
    torch_dataset_all = HSIDA(images=all_data, labels=torch.from_numpy(gt_all).type(torch.LongTensor),
                              labels_gt=torch.from_numpy(gt_all).type(torch.LongTensor),mode='test')
...
def load_dataset(Dataset):
    ...
    if Dataset == 'Houston':
        ...
        gt_Houston = sio.loadmat('./dataset/Houston_gt.mat')
        data_hsi = Houston['Houston']
        gt_hsi = gt_Houston['Houston_gt']
        TOTAL_SIZE = 15029

    if Dataset == 'Houston2018':
        Houston2018 = sio.loadmat('./dataset/Houston2018.mat')
        gt_Houston2018 = sio.loadmat('./dataset/Houston2018_gt.mat')
        data_hsi = Houston2018['HoutonU2018_img']
        gt_hsi = gt_Houston2018['HoutonU2018_gt']
        TOTAL_SIZE = 504856
    if Dataset == 'WHU_Hi_HanChuan':
        WHU_Hi_HanChuan = sio.loadmat('./dataset/WHU_Hi_HanChuan.mat')
        gt_WHU_Hi_HanChuan = sio.loadmat('./dataset/WHU_Hi_HanChuan_gt.mat')
        data_hsi = WHU_Hi_HanChuan['WHU_Hi_HanChuan']
        gt_hsi = gt_WHU_Hi_HanChuan['WHU_Hi_HanChuan_gt']
        TOTAL_SIZE = 504856
