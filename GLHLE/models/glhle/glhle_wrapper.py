...
class Wrapper(nn.Module):
    ...
    def __init__(self,
        ...
        # self.model_scratch = DCRN.DARN(input_channel, num_classes)
        self.model_scratch = MyNet.mynet(input_channel, num_classes)
    ...
    def forward_cls_loss(self,
        ...
        with torch.no_grad():
            labels = self.pseudo_labels[indices].long()
            ...
            mix_prob = F.softmax(mix_logits.detach(), dim=1)
            ...
            # WS增强
            targets_corrected0 = comb(w_prob, targets_onehot_noise, confidences * self.scale1)
            targets_mix_corrected = comb((q_prob1 + w_prob) * 0.5, targets_onehot_noise, confidences * self.scale2)
        ...
        if self.warmup:
            ...
            cls_loss1 = F.cross_entropy(w_logits, labels) + \
                        F.cross_entropy(q_logits1, labels)
            ...
            cls_loss1 = CE(q_logits1, targets_corrected0) + \
                        CE(w_logits, targets_corrected2)
            cls_loss2 = 0.5 * CE(mix_logits, targets_mix_corrected) + 0.5 * RCE(mix_logits, targets_mix_corrected)
...
class SimCLRWrapper(Wrapper):
    def __init__(self,
        ...
        super(SimCLRWrapper, self).__init__(input_channel=input_channel,
    ...
    def inference(self, im_w, im_q, im_k):
        ...
        im_mix, mix_randind, mix_lam = mixup(torch.cat([im_w, im_q]), alpha=self.mixup_alpha)
        q, q_logits = self.model_scratch(torch.cat([im_w, im_mix, im_q, im_k]))
    ...
    def forward_contrastive_loss(self, q_w, q1, q2, indices):
        contrastive_loss = self.loss(q_w, q1)
    ...
    def forward_loss(self, im_w, im_q, im_k, indices):
        ...
        contrastive_loss = self.forward_contrastive_loss(q_w, q1, q2, indices)
        ...
        ent_loss, ne_loss = self.forward_reg_loss(torch.cat([w_logits, q_logits1, logits_mix]))
